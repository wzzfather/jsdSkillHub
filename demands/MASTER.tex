...
\fancyhead[R]{\small\color{graytext} MASTER v1.4}
...
D-020 & 用户设置与个人信息 & 个人资料编辑、密码修改、界面偏好设置（DEMAND-016） & V1 & \textcolor{warning}{待开发} \\
\bottomrule
\end{tabular}
\end{table}
...
% ---------- D-020 ----------
\subsection{D-020：用户设置与个人信息管理}

\textbf{状态}：待开发。\textbf{优先级}：\textcolor{warning}{P1}。

\textbf{需求描述}：补充用户自管理功能，包括个人资料查看/编辑（用户名、邮箱）、密码修改、界面偏好设置（语言/主题）。前端新增 \texttt{/settings} 页面（左 Tab 右内容区布局），右上角下拉菜单新增「个人设置」入口。后端新增 \texttt{GET/PUT /api/auth/me} 和 \texttt{PUT /api/auth/change-password} 三个接口。

\textbf{对应文档}：DEMAND-016-user-settings。
...
版本 & 日期 & 变更内容 \\
\midrule
v1.4 & 2026-05-12 & 新增 D-020 用户设置与个人信息管理（DEMAND-016）\\
\midrule
