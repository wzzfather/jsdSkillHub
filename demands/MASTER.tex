...
\begin{tikzpicture}[
  ...
  \node[flowstep, right=1.6cm of n1] (n2) {PM 评估};
  \node[flowstep, right=1.6cm of n2] (n3) {拆解分配};
  \node[flowstep, right=1.6cm of n3] (n4) {Cursor 执行};
  \node[decision, right=1.4cm of n4] (n5) {PM\\验收};
  \node[flowstep, right=1.6cm of n5] (ok) {通过结案};
  ...
  \draw[arr] (n5.south) -- +(0,-0.42) -| node[pos=0.32, left, font=\scriptsize] {不通过} (n4.south);
\end{tikzpicture}
\caption{需求变更流程}
