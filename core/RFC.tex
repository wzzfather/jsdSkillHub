...
R-010 & \textcolor{success}{已决定} & MVP 阶段全部用户默认 admin 角色，D-015 后新增 email/email\_verified 字段区分认证状态 \\
\bottomrule
\end{tabularx}
\end{table}
...
当 RFC 与 MASTER 总需求文档内容不一致时，按以下规则处理：
\begin{enumerate}[nosep]
  \item \textbf{接口文本、技术细节等局部不一致}：以 MASTER 总需求文档为主，RFC 同步修正
  \item \textbf{涉及重大流程或业务流转路径的问题}：必须由魏子政拍板决定方向，PM 不得自行假设
  \item \textbf{讨论点状态变更}：需 MASTER 或魏子政拍板确认后更新
  \item 每次修正后，同步更新 MASTER 和 RFC 的版本号
\end{enumerate}
...
版本 & 日期 & 变更内容 \\
\midrule
v1.1 & 2026-05-06 & R-010描述修正（补充email认证）；新增文档一致性维护规则 \\
